...
function taskIcon(task) {
  ...
  if (["video","video_watch","youtube","manual"].includes(type) || /video|youtube|watch/.test(title)) return "play";
  if (["telegram","telegram_membership","link","external_link","url"].includes(type) || /telegram|channel|subscribe/.test(title)) return "users";
  if (type === "referral_count" || /referral/.test(title)) return "plusUser";
  if (String(task.category || "").toLowerCase() === "daily" || /daily|check.?in/.test(title)) return "calendar";
  return "gift";
}
...
function TaskCard({task,onOpen}) {
  ...
  const verified = !completed && ["verified"].includes(status);
  const started = !completed && !verified && ["started","in_progress"].includes(status);
  ...
  return (
    <article className="task-card">
      ...
      <div className="task-action">
        ...
        <button
          className={completed ? "completed-btn" : "go-btn"}
          onClick={() => !completed && onOpen(task)}
          disabled={completed}
        >
          {completed ? <><Icon name="check" size={17}/> Completed</> :
            verified ? <>Claim <Icon name="check" size={17}/></> :
            started ? <>Verify <Icon name="check" size={17}/></> :
...
export default function App() {
  ...
  async function loadApp() {
    ...
    const results = await Promise.allSettled([
      api("/api/auth/telegram",{method:"POST",body:JSON.stringify({initData})}),
      api("/api/me"),
      api("/api/dashboard"),
      api("/api/mining"),
      api("/api/tasks"),
    ...
    const [auth, me, dash, mine, taskData, refs, tx] = results;
    const firstError = results.find(r => r.status === "rejected");
    if (firstError) setError(firstError.reason?.message || "Some AeroMint data could not be loaded.");

    if (auth.status === "fulfilled") setUser(auth.value.user || null);
    else if (me.status === "fulfilled") setUser(me.value.user || me.value || null);

    if (dash.status === "fulfilled") setDashboard(dash.value);
    if (mine.status === "fulfilled") setMining(mine.value);
  ...
  function openExternalUrl(url) {
    ...
    try {
      ...
      if (/^(https?:\/\/)?(t\.me|telegram\.me)\//i.test(value)) {
        ...
        if (tg?.openTelegramLink) { tg.openTelegramLink(normalized); return true; }
      }
      const normalized = value.startsWith("http") ? value : `https://${value}`;
      if (tg?.openLink) { tg.openLink(normalized); return true; }
